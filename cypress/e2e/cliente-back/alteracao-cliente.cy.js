describe('Alteração de dados cadastrais do cliente', () => {
    ...
    function enviarFormulario() {
        cy.get('#formDadosPessoais').then(($form) => {
            $form[0].submit();
        });
    }

    function validarToastErro(mensagem) {
        cy.get('#toast', { timeout: 6000 }).should('be.visible').and('contain', mensagem);
    }

    // RF0022 - Alterar cliente
    it('CT01 - Deve alterar os dados cadastrais do cliente', () => {
        cy.get('[name="nome"]').clear().type('Bruno Henrique');
        cy.get('[name="telefone"]').clear().type('11987654321');
        cy.get('[name="genero"]').select('3');
        enviarFormulario();
        cy.get('#toast').should('be.visible').and('contain.text', 'Dados salvos com sucesso!');
    });

    // RF0022 - Alterar cliente
    it('CT02 - Deve alterar somente o nome do cliente', () => {
        capturarDados().then((dados) => {
            cy.get('[name="nome"]').clear().type('Bruno Henrique Lima');
            enviarFormulario();

            cy.get('[name="nome"]').should('have.value', 'Bruno Henrique Lima');
            cy.get('[name="cpf"]').should('have.value', dados.cpf);
            cy.get('[name="tipoTelefone"]').should('have.value', dados.tipoTelefone);
            cy.get('[name="telefone"]').should('have.value', dados.telefone);
            cy.get('[name="genero"]').should('have.value', dados.genero);
            cy.get('[name="dataNascimento"]').should('have.value', dados.dataNascimento);
        });
    });
    ...
    it('CT03 - Deve alterar somente o telefone do cliente', () => {
        capturarDados().then((dados) => {
            ...
            enviarFormulario();
            cy.reload();
            cy.get('[name="nome"]').should('have.value', dados.nome);
            cy.get('[name="cpf"]').should('have.value', dados.cpf);
            cy.get('[name="tipoTelefone"]').should('have.value', dados.tipoTelefone);
            cy.get('[name="telefone"]').should('have.value', '(11) 98765-4321');
            cy.get('[name="genero"]').should('have.value', dados.genero);
            cy.get('[name="dataNascimento"]').should('have.value', dados.dataNascimento);
        });
    });
    ...
    it('CT04 - Deve alterar somente o gênero do cliente', () => {
        capturarDados().then((dados) => {
            ...
            cy.get('[name="tipoTelefone"]').should('have.value', dados.tipoTelefone);
            cy.get('[name="telefone"]').should('have.value', dados.telefone);
            cy.get('[name="genero"]').should('have.value', '3');
            cy.get('[name="dataNascimento"]').should('have.value', dados.dataNascimento);
        });
    });
    ...
    it('CT05 - Deve alterar mais de um dado cadastral simultaneamente', () => {
        cy.get('[name="nome"]').clear().type('Bruno Henrique');
        cy.get('[name="telefone"]').clear().type('11987654324');
        cy.get('[name="genero"]').select('2');
        enviarFormulario();
        cy.get('#toast').should('be.visible').and('contain.text', 'Dados salvos com sucesso!');
    });
    ...
    it('CT06 - Não deve permitir alterar o CPF', () => {
        cy.get('[name="cpf"]').should('have.attr', 'readonly');
    });

    // RN0026 - Dados obrigatórios para o cadastro de um cliente
    it('CT07 - Não deve permitir alterar a data de nascimento', () => {
        cy.get('[name="dataNascimento"]').should('have.attr', 'readonly');
    });
    ...
    it('CT12 - Deve cancelar a alteração dos dados', () => {
        capturarDados().then((dados) => {
            cy.get('[name="nome"]').clear().type('Nome Que Não Deve Ser Salvo');
            cy.visit('/cliente/perfil');
            cy.get('[name="nome"]').should('have.value', dados.nome);
            cy.get('[name="cpf"]').should('have.value', dados.cpf);
            cy.get('[name="tipoTelefone"]').should('have.value', dados.tipoTelefone);
